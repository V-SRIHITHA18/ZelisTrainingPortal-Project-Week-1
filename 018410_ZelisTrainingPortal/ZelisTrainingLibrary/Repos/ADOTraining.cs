...
namespace ZelisTrainingLibrary.Repos
{
    public class ADOTraining : ITraining
    {
        ...
        {
            cmd.CommandText = $"select * from Tarining";
            con.Open();
        ...
        {
            cmd.CommandText = $"insert into Training values({training.TrainingId},{training.TrainerId},'{training.StartDate}', '{training.EndDate}')";
            con.Open();
            cmd.ExecuteNonQuery();
            con.Close();
        }
